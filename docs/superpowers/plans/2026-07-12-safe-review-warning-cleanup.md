...
**Architecture:** 运行时命令统一委托给已启用的 Feishu Lark CLI Sync 插件公开桥接，FloatMark 不再直接启动子进程；TypeScript 告警采用等价类型表达消除。根 README 保持中文并补充英文简介，完整英文说明保存在独立文件；CSS 只删除被同一条 `background: ... !important` 完全覆盖的严格冗余声明，其余主题兼容规则保持原样。
...
> 说明：本任务中的 README 入口交换步骤已被 Task 3 的用户反馈修正取代；Task 2 的 CSS 清理和验证仍然有效，最终 README 布局以 Task 3 为准。
...
### Task 3: 按用户反馈恢复中文主 README

**Files:**
- Modify/rename: `README.md`
- Create by rename: `README.en.md`
- Delete by rename: `README.zh-CN.md`
- Modify: `test/test-review-warnings.mjs`
- Modify: `docs/superpowers/specs/2026-07-12-safe-review-warning-cleanup-design.md`

- [x] 根 `README.md` 恢复为中文正文，并链接到 `README.en.md`。
