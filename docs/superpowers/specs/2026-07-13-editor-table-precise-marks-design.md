...
The widget host is `contenteditable=false`. CodeMirror's public `EditorView.posAtDOM()` maps the widget host directly to the complete Markdown table source range. No Obsidian private JavaScript API or custom Markdown cell parser is required.
...
- Do not add a custom Markdown table parser or depend on Obsidian private JavaScript APIs.
